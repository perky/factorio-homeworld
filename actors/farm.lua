...
function Farm:tick()
   ...
   if state.current_state == STATE_GROWING then
      if self:can_operate() then
         ...
         local interval = (1 / MINUTES) * state.current_production_interval
         local wheat_yield = math.floor(config.max_wheat_yield_per_min * interval * yield)
         local hops_yield = math.floor(config.max_hops_yield_per_min * interval * yield)
